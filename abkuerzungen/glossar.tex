...
\newglossaryentry{kryptologie}{
    name={Kryptologie},
    description={Oberbegriff für Kryptographie und Kryptoanalyse zur Sicherheit von Informationen und Entwicklung von Techniken zur Verschlüsselung und Entschlüsselung von Daten}
}
\newglossaryentry{steganographie}{
	name={Steganographie},
...
\newglossaryentry{monoalphabetisch}{
	...
	description={Verschlüsselungsart, bei der der Schlüssel für jedes Zeichen gleich bleibt}
}
\newglossaryentry{publicKey}{
	name = {public Key}, 
	description = {Ein öffentlich zugänglicher \glsdisp{kryptographie}{kryptographischer} Schlüssel, der für Verschlüsselung und Verifizierung verwendet wird}
}

\newglossaryentry{privateKey}{
	name = {private Key}, 
	description = {Ein geheimer \glsdisp{kryptographie}{kryptographischer} Schlüssel, der zur Entschlüsselung und Signierung von Daten dient und streng vertraulich bleiben muss}
}
\newglossaryentry{publicKeyEncoding}{
	name = {Public-Key Verschlüsselungsverfahren}, 
	description = {Eine Form der asymmetrischen Verschlüsselung, bei der ein \glsdisp{publicKey}{öffentlicher Schlüssel} zum Verschlüsseln von Daten verwendet wird, während der dazugehörige \glsdisp{privateKey}[geheimer Schlüssel] benötigt wird, um die verschlüsselten Daten zu entschlüsseln}
}
\newglossaryentry{bobAlice}{
	name = {Bob und Alice}, 
	description = {In der Kryptographie sind Bob und Alice fiktive Personen, die häufig verwendet werden, um die Kommunikation zwischen zwei Parteien zu veranschaulichen. Bob ist der Empfänger einer Nachricht oder einer verschlüsselten Botschaft, während Alice der Absender ist. Die Verwendung von Bob und Alice erleichtert das Verständnis und die Darstellung kryptografischer Konzepte und Protokolle}
}
...
\newglossaryentry{compressfunc}{
	...
	description = {In der Kryptographie eine Funktion, die Daten auf eine kompakte Form reduziert, wodurch Speicherplatz eingespart oder die Datenübertragungseffizienz verbessert wird}, 
	plural = {Kompressionsfunktionen}
}
\newglossaryentry{MD5}{
	name={Message-Digest Algorithm 5 (\acs{MD5})}, 
        plural = {\acs{MD5}},
	description = {Kryptographische \gls{hashfunc} zur berechnung eines 128-Bit-\gls{hashwert} aus einer beliebigen Nachricht}
}

\newglossaryentry{hashfunc}{
	name = {Hashfunktion}, 
	description= {Mathematische Funktion in der Kryptographie, die Daten beliebiger Länge auf eine feste Länge (\gls{hashwert}) abbildet und dabei eine eindeutige Identifizierung der Daten ermöglicht. Sie zeichnen sich durch \gls{kollRes} und \gls{unumkehrbarkeit} aus}, 
	plural = {Hashfunktionen}
}
\newglossaryentry{hashwert}{
	name = {Hashwert}, 
	plural = {Hashwerte}, 
	description = {Eine feste Länge von Bits, die durch Anwendung einer kryptographischen \gls{hashfunc} auf Daten erzeugt wird, um eine eindeutige Darstellung der Daten zu erhalten}
}
\newglossaryentry{kollRes}{
	name = {Kollisionsresistenz}, 
	plural = {Kollisionsresistenzen}, 
	description ={Die Eigenschaft in einer kryptograpsischen \gls{hashfunc}, in der es schwierig ist, zwei unterschiedliche Eingabewerte zu finden, die denselben \gls{hashwert} erzeugen}
}
\newglossaryentry{unumkehrbarkeit}{
	name={Unumkehrbarkeit}, 
	plural = {Unumkehrbarkeiten}, 
	description = {Die Eigenschaft einer kryptographischen \gls{hashfunc}, bei der es praktisch unmöglich ist, von einem gegebenn \gls{hashwert} auf den ursprünglichen Eingabewert zurückzuschließen}
}

\newglossaryentry{X509}{
        name = {X.509},
        description = {Sandard der \ac{ITUT} für eine \gls{publicKey}-Infrastruktur zum erstellen digitaler Zertifikate (\autoref{subsubsec:digitale-signaturen-und-zertifikate}). Im November 2020 als ISO/IEC 9594-8 aktualisiert worden}
}
...
\newglossaryentry{csm}{
        ...
        description = {Ein Architekturmodell, bei dem eine Kommnikation zwischen einem \gls{client}, der Anfragen stellt, und einem \gls{server}, der diese Anfragen bearbeitet und Antworten liefert, etabliert ist}
}
...
\newglossaryentry{vertraulichkeit}{
        ...
        description = {Daten dürfen nur von Personen verarbeitet werden, die dafür bestimmt sind}
}

\newglossaryentry{verfügbarkeit}{
        name = {Verfügbarkeit},
...
\newglossaryentry{authentifizierung}{
        ...
        description = {Der Prozess der Überprüfung der Identität eines \gls{client}s, Geräts oder Systems, um sicherzustellen, dass sie tatsächlich diejenige Partei sind, für die sie sich ausgeben}
}

\newglossaryentry{autorisierung}{
        name = {Authorisierung},
        plural = {Authorisierungen},
...
\newglossaryentry{Rainbow-Table}{
        ...
        description = {Eine vorausberechnete Tabelle, die zur schnellen Rückberechnung von \glspl{hashwert}n verwendet wird, um Passwörter oder andere ursprüngligche Daten zu entschlüsseln, indem sie vorab \glspl{hashwert} mit den entsprechenden Eingabewerten speichert}
}

\newglossaryentry{malware}{
        name = {Malware},
        plural = {Malwares},
        description = {Eine bösartige Software, die entwickelt wurde, um unerwünschte oder schädliche aktionen auf einem Computer oder einem Netzwerk auszuführen, wie z.B. Datenverlust, Systembeschädigung oder unbefugten Zugriff ermöglichen}
}

\newglossaryentry{sqlInject}{
        name = {SQL-Injection},
        plural = {SQL-Injections},
        description = {Eine Angriffstechnik, bei der schädlicher SQL-Code in eine Andwendungsdatenbank eingeschleust wird, um unautorisierten Zugriff auf die Datenbank zu erlangen oder unerwünschte Aktionen durchzuführen}
}
...
\newglossaryentry{SHA256}{
        name = {SHA256},
        description = {Ein \glsdisp{kryptographie}{kryptographischer} \glsdisp{hashfunc}{Hash}\glsdisp{algorithmus}{algorithmus}, der eine 256\nobreakdash-Bit-\gls{hashfunc} verwendet, um eine eindeutige Darstellung von Daten zu erzeugen und häufig für die Integritätsprüfung und digitale Signatur verwendet wird}
}