...
\autoref{tab:vigenere}\autocite[\pagef 6]{Kryptografische_algorithmen} zeigt das Vigènere-Verfahren mit dem Schlüssel \enquote{ALICE} auf dem Klartext \enquote{DIES IST EIN VERSCHLÜSSELTER TEXT}
...
\footnotetext{\cite[S. 6]{Kryptografische_algorithmen}}
...
Den anderen \glsdisp{kryptografie}{Kryptografiemaschinen} ihrer Zeit war die Enigma um ein Vielfaches überlegen.\\
...
Symmetrische Verschlüsselungsverfahren werden deshalb als sicher anerkannt, wenn man ohne den Schlüssel den Klartext nicht aus dem verschlüsselten Text ermitteln kann.\autocite[\pagef~5]{Kryptografische_algorithmen}
...
Damit Hash- und \glspl{compressfunc} in der \gls{kryptografie} zur Authentifizierung, wie z.B.\ zur Speicherung von Passwörtern genutzt werden können, müssen noch ein paar Kritierien erfüllt werden.
...
\begin{definition}
    ...
    \end{enumerate}
    \footnotetext{\enquote{einfach} und \enquote{schwer} sind hier im Kryptografischen Sinne zu verstehen und beziehen sich auf das Zusammenspiel von Laufzeit und Rechenaufwand eines \gls{algorithmus}}
\end{definition}
